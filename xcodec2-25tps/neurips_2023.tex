...
Evaluated on the multilingual Common Voice 17 test set, the proposed configuration achieves a 0.5 MOS improvement over the original X-Codec-2.0 baseline. The source code, checkpoints and generation comparison released at \href{https://huggingface.co/malaysia-ai/xcodec2-25TPS-24k}{huggingface.co/malaysia-ai/xcodec2-25TPS-24k}.
...
Empirically, the proposed model achieves a +0.5 improvement in mean opinion score (MOS) as estimated by UTMOSv2~\cite{baba2024t05voicemoschallenge2024} on the multilingual Common Voice~\cite{ardila2020commonvoicemassivelymultilingualspeech}~17 test set.  
...
We further benchmark our model against several recent neural audio codecs: SNAC~\cite{siuzdak2024snac}, Encodec~\cite{défossez2022highfidelityneuralaudio}, DAC~\cite{kumar2023highfidelityaudiocompressionimproved}, Mimi~\cite{défossez2024moshispeechtextfoundationmodel}, BigCodec~\cite{xin2024bigcodecpushinglimitslowbitrate}, SpeechTokenizer~\cite{zhang2024speechtokenizerunifiedspeechtokenizer}, WavTokenizer~\cite{ji2025wavtokenizerefficientacousticdiscrete}, BiCodec~\cite{wang2025sparkttsefficientllmbasedtexttospeech}, UniCodec~\cite{jiang2025unicodecunifiedaudiocodec}, DistilCodec~\cite{wang2025unittsendtoendttsdecoupling}, Neucodec~\cite{julian2025finitescalarquantizationenables} and X-Codec-2.0 baseline.
