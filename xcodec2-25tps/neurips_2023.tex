...
pdfauthor={Malaysia-AI Volunteer},
...
The dataset combines publicly available TTS and expressive speech corpora from Hugging Face, covering over 20 languages such as English, Mandarin, Malay, Japanese, Korean, Arabic, Hindi, Tamil, Bengali, and several European languages.
Each audio sample is uniformly resampled to 24\,kHz and cropped into randomly selected 5-second segments.
No text transcriptions are used during training. The full list of datasets is publicly available at \href{https://huggingface.co/datasets/malaysia-ai/Multilingual-TTS}{malaysia-ai/Multilingual-TTS}.
...
\section{Evaluation}
We evaluate perceptual quality using the UTMOSv2 model-based speech quality estimator, a neural predictor trained to approximate human mean opinion scores (MOS) across multiple languages and acoustic conditions. 
UTMOSv2 extends the original UTMOS by incorporating multilingual training data and a more robust calibration pipeline, making it suitable for evaluating speech codecs and vocoders without subjective listening tests.

\subsection{Evaluation Dataset}
We use the multilingual Common Voice~22 test set, which covers approximately 100 languages. 
For each language, all available utterances are sorted by duration, and the 500 longest clips are selected, resulting in a total of 50,000 evaluation samples. We uploaded the test set samples at \href{https://huggingface.co/malaysia-ai/xcodec2-25TPS-24k}{malaysia-ai/xcodec2-25TPS-24k}.
...
We report UTMOSv2 results on eight representative languages, Spanish, German, English, French, Italian, Polish, Dutch, and Portuguese which collectively cover a range of phonetic and prosodic diversity among high-resource European languages. Results for the complete 100-language evaluation are provided in Appendix~\ref{appendix:full-eval}.
...
In this section, we provide the full UTMOSv2 evaluation results across 100 languages.
